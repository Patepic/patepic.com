...
import { reviews, PLATFORMS, GENRES } from "../data/reviews";
...
export default function Reviews() {
  ...
  const filtered = useMemo(() => {
    ...
    return res;
  }, [query, selectedPlatforms, selectedGenres, scoreRange, sort]);
  ...
  return (
    <div data-testid="reviews-page" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 lg:py-20">
      ...
      <div className="mb-12">
        ...
        <h1 className="font-display text-4xl sm:text-5xl lg:text-6xl text-slate-50 tracking-tighter">
      ...
      <div className="flex flex-col md:flex-row gap-3 mb-8">
        <div className="relative flex-1">
          ...
          <Input
            ...
            className="pl-11 h-12 bg-slate-900 border-slate-800 text-slate-100 placeholder:text-slate-500 focus-visible:ring-cyan-500"
          />
        </div>
        ...
        <Select value={sort} onValueChange={setSort}>
          <SelectTrigger
            ...
            className="md:w-56 h-12 bg-slate-900 border-slate-800 text-slate-100"
          >
            <SelectValue />
          </SelectTrigger>
        ...
        <button
          ...
          className="md:hidden inline-flex items-center justify-center gap-2 h-12 px-4 rounded-lg bg-slate-900 border border-slate-800 text-sm"
        >
          <SlidersHorizontal className="w-4 h-4" /> Filters
          {activeCount > 0 && (
            <span className="ml-1 px-2 py-0.5 rounded-full bg-cyan-500 text-slate-950 text-xs font-medium">
              {activeCount}
            </span>
          )}
        </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
        ...
        >
          <div className="flex items-center justify-between">
            <h3 className="font-display text-lg text-slate-50">Filters</h3>
            {activeCount > 0 && (
              <button
                onClick={clearAll}
            ...
            {PLATFORMS.map((p) => (
              <label key={p} className="flex items-center gap-2.5 text-sm cursor-pointer group">
                <Checkbox
                  ...
                  className="border-slate-600 data-[state=checked]:bg-cyan-500 data-[state=checked]:border-cyan-500 data-[state=checked]:text-slate-950"
                />
                <span className="text-slate-300 group-hover:text-slate-100">{p}</span>
              </label>
            ))}
          </FilterGroup>
            ...
            {GENRES.map((g) => (
              <label key={g} className="flex items-center gap-2.5 text-sm cursor-pointer group">
                <Checkbox
                  ...
                  className="border-slate-600 data-[state=checked]:bg-cyan-500 data-[state=checked]:border-cyan-500 data-[state=checked]:text-slate-950"
                />
                <span className="text-slate-300 group-hover:text-slate-100">{g}</span>
              </label>
            ))}
          </FilterGroup>
        ...
        <div className="md:col-span-9">
          <div className="mb-5 text-sm text-slate-400" data-testid="reviews-result-count">
            Showing <span className="text-slate-100 font-medium">{filtered.length}</span> of {reviews.length} reviews
          </div>

          {filtered.length === 0 ? (
            <div className="border border-dashed border-slate-800 rounded-2xl p-16 text-center">
              <p className="text-slate-300 font-display text-xl mb-2">No reviews match those filters.</p>
              <p className="text-slate-500 text-sm">Try widening the score range or clearing filters.</p>
            </div>
          ) : (
...
const FilterGroup = ({ label, children }) => (
  <div>
    <p className="text-xs tracking-[0.2em] uppercase text-cyan-500 mb-4">{label}</p>
